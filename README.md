...
Since the Hydra managed to crack the password of the cisco user we have managed to gain the access to the Ubuntu machine. The regular user, however, does not have an access to system files (e.g. /etc/shadow), but in this case cisco user have root privileges.
...
The command we used afterwards was: "sudo john --single unshadowed_hashes.txt" to load the text file and "john --show unshadowed_hashes.txt" to display the password for specific account.
- sudo john: runs john application with root privileges
- --single unshadowed_hashes.txt: single mode targets passwords using contextual clues available within the target user account profile
- --show unshadowed_hashes.txt: displays the gained information
